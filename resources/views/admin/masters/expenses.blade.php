                            ...
                            <tbody>
                                @foreach ($expenses as $expenses)
                                    <tr>
                                        ...
                                        <td>
                                            @can('expenses.edit')
                                                <button class="edit-element btn btn-soft-secondary px-0 py-0" title="Edit expenses" data-id="{{ $expenses->id }}"><i class="ri-edit-box-line fs-4 w-100 h-100 text-center"></i></button>
                                            @endcan
                                            @can('expenses.delete')
                                                <button class="delete-element btn btn-soft-danger rem-element px-0 py-0" title="Delete expenses" data-id="{{ $expenses->id }}"><i class="ri-delete-bin-2-line fs-4 w-100 h-100 text-center"></i> </button>
                                            @endcan
...
<script>
   $("#buttons-datatables").on("click", ".edit-element", function(e) {
    ...
    var url = "{{ route('expenses.edit', ':model_id') }}";
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
            editFormBehaviour();
            if (!data.error) {
                $("#editForm input[name='edit_model_id']").val(data.expenses.id);
                $("#edit_name").val(data.expenses.name);
                $("#edit_initial").val(data.expenses.initial);
                $("#edit_status").val(data.expenses.status);
            } else {
                alert(data.error);
            }
        },
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('expenses.update', ':model_id') }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
